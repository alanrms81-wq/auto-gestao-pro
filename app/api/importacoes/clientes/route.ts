...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!clientes.length) {
      return NextResponse.json({ error: "NENHUM CLIENTE INFORMADO." }, { status: 400 });
    }

    const admin = getAdminClient();

    const payload: ClientePayload[] = clientes
      .map((item: any): ClientePayload => ({
      ...
      .filter((item: ClientePayload) => item.nome);
    ...
    if (error) {
      return NextResponse.json(
        { error: "ERRO AO IMPORTAR CLIENTES.", detalhe: error.message },
        { status: 400 }
      );
    }
