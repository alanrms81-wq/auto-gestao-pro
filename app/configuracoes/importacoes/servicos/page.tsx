...
export default function ImportacaoServicosPage() {
  ...
  async function importar() {
    ...
    try {
      ...
      const servicos = linhas.map((row) => ({
        nome: String(row.nome || row.servico || row.serviço || "").trim().toUpperCase(),
        descricao: String(row.descricao || row.descrição || "").trim().toUpperCase(),
        valor: Number(String(row.valor || row.preco || row.preço || 0).replace(",", ".")) || 0,
        status: "ATIVO",
      })).filter((s) => s.nome);
      ...
      if (!res.ok) {
        alert(json.error || "ERRO AO IMPORTAR SERVIÇOS.");
        return;
      }
  ...
  function baixarModelo() {
    const csv =
      ...
      "TROCA DE OLEO,TROCA COMPLETA DE OLEO E FILTRO,120.00\n";
              ...
              <table className="tabela min-w-[800px]">
                ...
                <tbody>
                  {linhas.slice(0, 50).map((row, i) => (
                    <tr key={i}>
                      <td>{row.nome || row.servico || row.serviço || "-"}</td>
                      <td>{row.descricao || row.descrição || "-"}</td>
                      <td>{row.valor || row.preco || row.preço || "-"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
