...
export default function ImportacaoClientesPage() {
  ...
  async function importar() {
    ...
    try {
      ...
      const clientes = linhas.map((row) => ({
        nome: String(row.nome || row.cliente || "").trim().toUpperCase(),
        ...
        endereco: String(row.endereco || row["endereço"] || "").trim().toUpperCase(),
        numero: String(row.numero || row["número"] || "").trim().toUpperCase(),
        bairro: String(row.bairro || "").trim().toUpperCase(),
      ...
      if (!res.ok) {
        alert(json.error || "ERRO AO IMPORTAR CLIENTES.");
        return;
      }
  ...
  function baixarModelo() {
    const csv =
      "nome,telefone,celular,whatsapp,cpf_cnpj,email,endereco,numero,bairro,cidade,uf,cep,observacoes\n" +
