...
export default function ImportacaoProdutosPage() {
  ...
  async function importar() {
    ...
    try {
      ...
      const produtos = linhas.map((row) => ({
        nome: String(row.nome || row.produto || "").trim().toUpperCase(),
        codigo_sku: String(row.codigo || row.sku || "").trim().toUpperCase(),
        preco_balcao: Number(String(row.preco || row.valor || 0).replace(",", ".")) || 0,
        estoque_atual: Number(row.estoque || row.quantidade || 0) || 0,
      ...
      if (!res.ok) {
        alert(json.error || "ERRO AO IMPORTAR PRODUTOS.");
        return;
      }
  ...
  function baixarModelo() {
    const csv =
      "nome,codigo,preco,estoque,categoria\n" +
      "FILTRO DE OLEO,FO123,25.90,10,MOTOR\n";
    ...
    <div className="min-h-screen flex bg-[#F3F4F6]">
      ...
      <main className="flex-1 min-w-0 p-3 md:p-6">
        <div className="flex flex-col md:flex-row justify-between gap-4 mb-6">
          <div>
            <h1 className="text-[26px] font-black text-[#6C757D]">IMPORTAÇÃO DE PRODUTOS</h1>
            <p className="text-sm text-[#6C757D] mt-1">IMPORTE PRODUTOS VIA CSV</p>
          </div>
              ...
              <table className="tabela min-w-[900px]">
                ...
                <tbody>
                  {linhas.slice(0, 50).map((row, i) => (
                    <tr key={i}>
                      <td>{row.nome || row.produto || "-"}</td>
                      <td>{row.codigo || row.sku || "-"}</td>
                      <td>{row.preco || row.valor || "-"}</td>
                      <td>{row.estoque || row.quantidade || "-"}</td>
                      <td>{row.categoria || "-"}</td>
                    </tr>
                  ))}
                </tbody>
