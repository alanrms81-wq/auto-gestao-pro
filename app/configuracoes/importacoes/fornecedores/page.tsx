...
function converterFornecedores(rows: Record<string, string>[]) {
  return rows
    .map((row) => ({
      nome: up(
        pick(row, [
      ...
      cpf_cnpj: digits(pick(row, ["cpf_cnpj", "cpf/cnpj", "cnpj", "documento"])),
      email: String(pick(row, ["email", "e-mail"])).trim().toLowerCase(),
      contato: up(pick(row, ["contato", "responsavel", "responsável"])),
...
export default function ImportacaoFornecedoresPage() {
  ...
  async function processarArquivo(file: File) {
    ...
    Papa.parse<Record<string, unknown>>(texto, {
      ...
      complete: (result: ParseResult<Record<string, unknown>>) => {
        ...
        setFornecedores(converterFornecedores(dadosNormalizados));
      },
      error: (error: Error) => {
  ...
  async function importar() {
    ...
    try {
      ...
      if (!res.ok) {
        alert(
          `${json.error || "ERRO AO IMPORTAR FORNECEDORES."}${
  ...
  function baixarModelo() {
    const csv =
      "Fornecedor;Telefone;Celular;WhatsApp;CNPJ;Email;Contato;Endereco;Numero;Bairro;Cidade;UF;CEP;Observacoes\n" +
      "AUTO PECAS BRASIL;1133334444;11999998888;11999998888;12345678000199;contato@autopecas.com;JOSE;RUA B;200;CENTRO;SAO PAULO;SP;01002000;FORNECEDOR PRINCIPAL\n";
        ...
        <section className="card mb-6">
          ...
          {linhasLidas > 0 ? (
            <div className="mt-2 text-sm text-[#374151] space-y-1">
              ...
              <p>FORNECEDORES VÁLIDOS: <b>{fornecedores.length}</b></p>
          ...
          <section className="card">
            ...
            <div className="overflow-x-auto">
              <table className="tabela min-w-[1200px]">
                ...
                <tbody>
                  {fornecedores.slice(0, 100).map((item, i) => (
